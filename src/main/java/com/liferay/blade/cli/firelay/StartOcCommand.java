...
public class StartOcCommand extends BaseCommand<StartOcCommandArgs> {
	...
	private void verifyEnvs() {
		if (!FireBladeConfUtil.checkMinishift()) {
			System.out.println("Minishift isn't installed, please download the latest version of Minishift "
					+ "if you want to run your project locally:");
			System.out.println("https://github.com/minishift/minishift");
			System.out.println("Or you can also configure your openshift properties on "
					+ "/configs/oc/env.properties to run it remotly");
			return;
		}
		if (!FireBladeConfUtil.checkOc()) {
			System.out.println("The command oc was not found.  Make sure it is correctly configured.  "
					+ "You can run 'minishift oc-env' and follow the instructions to configure it.");
			return;
		}
		startMinishift();
	}
	...
	private void startMinishift() {
		try {
			...
			_trace(cmdString);
			Process process = BladeUtil.startProcess(getBladeCLI(), "\"" + minishiftCommand + "\" " + cmdString, null,
					false);
			process.waitFor();
		} catch (Exception e) {
			e.printStackTrace();
	...
	private void loginAsAdmin() {
		...
		_trace(cmdString);
		try {
			Process process = BladeUtil.startProcess(getBladeCLI(), "\"" + "oc" + "\" " + cmdString, null, false);
			process.waitFor();
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	...
	private void addProject(boolean isRemote) {
		...
		if (isRemote) {
			cmdString = "new-project " + getBladeCLI().getBase().getName();
		}
		try {
			Process process = BladeUtil.startProcess(getBladeCLI(), "\"" + "oc" + "\" " + cmdString, null, false);
			process.waitFor();
	...
	private String projectName() {
		return getBladeCLI().getBase().getName();
	}

	private void initMySqlPod() {
		System.out.println("*** Deploying MySQL ***");
		File mysql = new File(getBladeCLI().getBase().getAbsolutePath() + "/configs/oc/mysql.yaml");
		if (mysql.exists()) {
			String cmdString = "process -f " + mysql.getAbsolutePath() + " -p DEPLOYMENT_NAME=" + projectName()
					+ " | oc apply -f -";
			try {
				Process process = BladeUtil.startProcess(getBladeCLI(), "\"" + "oc" + "\" " + cmdString, null, false);
				process.waitFor();
	...
	private void initLiferayPod() {
		...
		File propertiesFile = new File(getBladeCLI().getBase().getAbsolutePath() + "/configs/oc/env.properties");
		try {
			...
			if (propertiesFile.exists()) {
				...
				File liferay = new File(getBladeCLI().getBase().getAbsolutePath() + "/configs/oc/liferay7.yaml");
				if (liferay.exists()) {
					String cmdString = "process -f " + liferay.getAbsolutePath() + " -p DEPLOYMENT_NAME="
							+ projectName() + " -p GITHUB_URL=" + gitHubUrl + " -p GITHUB_BRANCH=" + gitHubBranch
							+ " | oc apply -f -";
					try {
						Process process = BladeUtil.startProcess(getBladeCLI(), "\"" + "oc" + "\" " + cmdString, null,
								false);
						process.waitFor();
					} catch (Exception ex) {
						ex.printStackTrace();
	...
	private void initJenkins() {
		...
		File jenkins = new File(getBladeCLI().getBase().getAbsolutePath() + "/configs/oc/jenkins.yaml");
		if (jenkins.exists()) {
	...
	private void initElasticSearch() {
		...
		File es = new File(getBladeCLI().getBase().getAbsolutePath() + "/configs/oc/elasticsearch.yaml");
		if (es.exists()) {
			String cmdString = "process -f " + es.getAbsolutePath() + " -p DEPLOYMENT_NAME=" + projectName()
					+ " | oc apply -f -";
			try {
				Process process = BladeUtil.startProcess(getBladeCLI(), "\"" + "oc" + "\" " + cmdString, null, false);
				process.waitFor();
