...
import com.liferay.blade.cli.command.*;
...
public class DeployCommand extends BaseCommand<DeployOcCommandArgs> {
	...
	public void execute() throws Exception {
		...
		if (_verifyPodStatus()) {
			...
			Set<File> outputFiles = GradleTooling.getOutputFiles(getBladeCLI().getCacheDir(), getBladeCLI().getBase());
			if (getArgs().isWatch()) {
	...
	private Boolean _verifyPodStatus() {
		...
		String line = "";
		try {
			String[] arguments = new String[] { "oc", "get", "pods", "-o",
					"jsonpath='{.items[0].status.containerStatuses[0].ready}'", "-l", "tier=portal" };
	...
	private void _deploy(GradleExec gradle, Set<File> outputFiles) throws Exception {
		...
		stream.filter(File::exists).forEach(outputFile -> {
			try {
	...
	private void _deployWatch(final GradleExec gradleExec, final Set<File> outputFiles) throws Exception {
		...
		_deploy(gradleExec, outputFiles);
		Stream<File> stream = outputFiles.stream();
		Collection<Path> outputPaths = stream.map(File::toPath).collect(Collectors.toSet());
	...
	private void uploadToPod(File bundle) {
		...
		String podName = getArgs().getPodName();
	...
	private void checkForRemote() {
		...
		File propertiesFile = new File(getBladeCLI().getBase().getAbsolutePath() + "/configs/fireBlade/env.properties");
		try {
	...
	private void remoteLogin(String host, String user, String pass) {
		...
		String line = "";
		try {
			String[] arguments = new String[] { "oc", "login", host, "-u", user, "-p", pass,
